...
const transactions = computed(() => transactionStore.historyTransaction?.transactions)
const totalPage = computed(() => transactionStore.historyTransaction?.totalPages)
...
const searchHistoryByNickName = async () => {
  if (nickname.value) {
    await transactionStore.getHistoryTransaction(nickname.value, page.value, limit)
  }
};
    ...
    <table class="table">
      ...
      <tbody class="body">
        <tr class="row" v-for="item in transactions" :key="item._id">
          ...
          <td class="cell">{{ maskNumber(item.accountNumberClient) }}</td>
          <td class="cell">{{ maskNumber(item.transId) }}</td>
